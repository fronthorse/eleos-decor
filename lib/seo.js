...
export function normalizeSchemaPrice(price) {
...
export function absoluteHttpsUrl(path = "/") {
...
export function buildProductMerchantSchema(product, reviews = []) {
  const title = normalizeSeoText(product?.title);
  const productUrl = product?.id ? absoluteUrl(`/product/${product.id}`) : "";
  const images = getProductSchemaImages(product);
  const price = normalizeSchemaPrice(product?.price);

  if (!title || !productUrl || images.length === 0 || !price) {
    return null;
  }
  ...
  return {
    ...
    ...buildProductReviewSchema(reviews),
  };
}
