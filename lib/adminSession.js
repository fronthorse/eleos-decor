...
function getPublicSupabaseConfigSummary() {
  ...
  return {
    ...
    anonKeyPrefix: anonKey ? `${anonKey.slice(0, 6)}...` : "",
  };
}
...
export async function verifyAdminSession(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    logAdminAuthDebug("checking session", { source, attempt });
    ...
    if (sessionError) {
      ...
      if (
        isRecoverableAdminAuthError(sessionError) &&
        attempt < maxAttempts
      ) {
    ...
    logAdminAuthDebug("admin email check", {
      ...
      adminEmails: adminSnapshot.adminEmails,
    ...
    const { session: latestSession } = await getSessionSafely(supabase, {
      timeoutMs,
      timeoutMessage: "Admin session refresh check timed out.",
    });
