...
function getIntentLabel(intentSource) {
  if (intentSource === "gemini") {
    return "Intent: Gemini";
  }

  if (intentSource === "fallback") {
...
export default function MessageBubble({ message, onOptionSelect, isThinking }) {
  ...
  return (
    <>
      ...
      {debugLabel && (
        <div className="ai-assistant-debug-label">{debugLabel}</div>
      )}
    </>
  );
}
